	...
	if (strcmp(argv[0], "./mmm") == 0 && argv[1] != NULL) {
		...
		if (strcmp(argv[1], "S") == 0 && argv[3] == NULL) {
			...
			if (argv[2] != NULL) {
				...
				// Error
				printf("\nIncorrect parameters. Try ./mmm S <size of matrices>\n");
			}
			...
			if (argv[2] != NULL && argv[3] != NULL && argv[4] == NULL) {
					...
					if (*check == '\0' || check != argv[2]) {
						...
						if (*check == '\0' || check != argv[3]) {
							if (numThreads <= size) {
								...
								// Error
								printf("\nAmount of threads cannot be greater than the size!\n");
							}
						} else {
							// Error
							printf("\n%s is not a valid size.\n", argv[3]);
				...
				// Error
				printf("\nIncorrect parameters. Try ./mmm P <# of threads> <size of matrices>\n");
			}
		}
	} else {
		printf("\nIncorrect parameters. Try ./mmm <P|S>\n");
	}
	return 0;
}
